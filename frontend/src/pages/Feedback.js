...
const ManageFeedback = () => {
  ...
  const handleReply = async (parentId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await api.post(`/hostel_owner/feedback/`, {
        comment: replyText[parentId],
        parent: parentId, //  This is the key part
      }, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const mostRecentDate = feedback.length
    ...
    const renderReplies = (replies) => {
      return replies.map((reply) => (
        <div key={reply.id} className="reply-bubble-wrapper">
...
const handleDeleteReply = async (replyId) => {
  ...
  try {
    const token = localStorage.getItem("token");
    await api.delete(`/hostel_owner/feedback/${replyId}/delete-reply/`, {
      headers: { Authorization: `Bearer ${token}` },
    });
